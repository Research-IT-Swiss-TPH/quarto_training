...
While `reticulate` attempts to convert dataframes automatically, using the dictionary-to-list method is the most robust way to ensure compatibility with R's kable and dplyr functions, especially when dealing with complex Pandas indices or data types.

```{r}
#| label: tbl-2
#| tbl-cap: Data frame read from a Python object and displayed using R

df_from_python <- py$df2$to_dict("list") |> 
  reticulate::py_to_r() |> 
  as.data.frame() 
