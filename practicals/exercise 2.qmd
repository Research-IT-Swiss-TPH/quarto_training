...
Obviously you may not want your final document to display code. You can add options to executable code like this. For example, the `echo: false` option disables the printing of code (only output is displayed).
...
If you render the document with one or more chunks with errors, it will fail. It's always necessary to make sure that code chunks can successfully run in their order before rendering the document. Try to uncomment `something <- "HELLO WORLD!"` and render again.
