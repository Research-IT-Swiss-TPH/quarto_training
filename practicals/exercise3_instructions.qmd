...
> "***cross-reference** provides a summary of the overall population characteristics*"
...
> "*while **cross-reference** compares the characteristics of individuals who died versus those who are still alive*"
...
c.  Add a caption to the code chunk ;
d.  Assign a label to the code chunk ;
e.  Replace the placeholder text in bold with a cross-reference to the code chunk. 
...
c.  Actually you were given the wrong dataset. Update the report to use the dataset stored in `df2.RData` instead of the one stored in `df1.RData`?
d. Can you render 2 different reports
